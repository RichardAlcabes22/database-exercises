...
*/
-- FUNCTIONS_EXERCISE BEGINS HERE

-- 1. Done

-- 2. last_name stars and ends with 'E'. CONCAT() to show first and last name as single column output.
SELECT first_name, last_name, CONCAT(first_name, ' ',last_name) AS full_name
 FROM employees
 WHERE UPPER(last_name) LIKE 'E%E';
 
 -- 3. convert #2 full_names to all UPPER
 SELECT  UPPER(CONCAT(first_name, ' ',last_name)) AS full_name
 FROM employees
 WHERE UPPER(last_name) LIKE 'E%E';
 
 -- 4. all employees hired in 90's and born XMas. Datediff and find # days working for company
